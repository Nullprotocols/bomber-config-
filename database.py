# database.py
# Database functions for Telegram Bomber Bot
# Tables: users, protected_numbers
...
def init_db():
    ...
    c.execute('CREATE INDEX IF NOT EXISTS idx_protected_number ON protected_numbers(number)')
...
def get_user_target(user_id: int) -> Optional[str]:
    """Retrieve stored target phone number for a user."""
    conn = get_connection()
    c = conn.cursor()
...
def update_user_target(user_id: int, target: Optional[str]):
    """Store target phone number for a user."""
    conn = get_connection()
    c = conn.cursor()
...
def update_user_phone(user_id: int, phone: str):
    """Store user's own phone number (for self-bomb prevention)."""
    conn = get_connection()
    c = conn.cursor()
...
def get_recent_users_paginated(page: int, per_page: int = 10, days: int = 7) -> List[Dict]:
    """Return a page of users who joined in last N days, ordered by join date descending."""
    offset = page * per_page
    conn = get_connection()
    c = conn.cursor()
    c.execute('''
...
def get_all_protected_numbers() -> List[str]:
    """Return list of all protected numbers, ordered newest first."""
    conn = get_connection()
    c = conn.cursor()
